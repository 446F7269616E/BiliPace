...
BiliFocus 的单一用途是：**帮助用户在哔哩哔哩站内按自己设定的规则减少分心，并在本机查看使用时间统计。**
...
Chrome Web Store 的“最小权限”是要求：无论必选还是可选权限，都只能申请当前功能所需的最窄范围；不能为未来功能预留权限。Chrome 也建议可行时使用运行时可选权限。BiliFocus 的发布预算如下：
