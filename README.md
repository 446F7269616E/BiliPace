...
一个本地优先、开源、跨浏览器的 Bilibili 专注扩展。你可以隐藏推荐、评论、相关视频等干扰内容，为不同页面安排专注时段，并通过实时状态与日 / 周 / 月使用洞察了解时间去向。
...
- 开启 BewlyBewly! Ave Mujica 时，核心计时、专注拦截与计划模式仍可用，并保留搜索联想、标题规则和搜索快捷键；
...
- 以“专注中心”为一级入口，在观看清单、使用洞察与专注设置之间统一跳转；
...
2. 打开“专注中心”查看实时计时、今日时长与各功能入口；
3. 在“专注设置”中分别选择要隐藏的干扰内容，以及要整页拦截的板块、每日额度和详细计划；
4. 在“观看清单”中按需开启计划模式，添加视频并调整顺序；只有点击“开始观看”的对应视频会在设定时长内打开；
5. 被整页规则拦截时可返回，或在允许次数内临时访问；
6. 打开“使用洞察”查看日 / 周 / 月统计，也可以清空所有本地统计。
