...
- 支持精确域名黑名单/白名单、元素隐藏、CSS 与安全声明式网络规则；
...
Hourleaf 商店包只提供通用、可解释的专注核心：计时、精确网站权限、域名策略、元素隐藏、CSS、安全 DNR 规则、模块导入器和平台允许的用户脚本隔离环境。商店包不内置 Bilibili 或其他网站的专用可执行模块。

GitHub 的 [可选模块目录](./optional-modules/) 只提供供用户查看和下载的普通文件。Hourleaf 不会联网读取目录、检查模块更新或根据 URL 执行代码。用户需要先阅读文件，再在“设置 → 模块设置”中手动选择本地文件、核对网站范围并授权。

模块清单只接受精确 HTTP(S) 来源。DNR 仅允许 `block`、`allow`、`upgradeScheme`，并由核心强制绑定模块声明的网站；CSS 禁止远程 `@import`；用户脚本不会通过 `eval`、`Function`、script 标签或通用 scripting API 执行。
...
- Safari App Store：根据 Apple [App Review Guidelines 2.5.2](https://developer.apple.com/app-store/review/guidelines/) 禁止导入/执行用户脚本，只保留 JSON、CSS 和平台支持的 DNR 子集。
